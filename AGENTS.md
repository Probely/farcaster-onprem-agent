...
Build outputs in `farcaster-go/bin/`, `farconn/farconn`, and `contrib/proxyprobe/bin/` are ignored by Git.

- Keep tokens, WireGuard private keys, downloaded agent configuration, and logs containing them out of commits, even if their filenames are not ignored.
- Container files under `/run/farcaster` are runtime configuration. Change the code that generates them.
- Let Go update `go.sum` when dependencies change. Do not edit checksums by hand or change dependency files during unrelated work.
...
- Use lowercase, imperative commit subjects such as `docs: add repository guide`. Use `type: summary` or `type: scope summary`, without parentheses or a trailing period, and keep the subject within 72 characters.
