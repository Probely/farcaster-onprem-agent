...
func testWebSocketTransport(wsURL *url.URL, payload []byte, insecure bool, payloadName string) error {
	...
	if os.Getenv("HTTP_PROXY") != "" {
		...
		proxyType = fmt.Sprintf("socks5 proxy: %s", os.Getenv("SOCKS5_PROXY"))
	}
...
func main() {
	...
	if httpProxy != "" {
		// Make sure the proxy URL starts with http:// or https://
	...
	if socks5Proxy != "" {
		// Make sure the proxy URL starts with socks5://
